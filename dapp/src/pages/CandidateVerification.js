...
export default function CandidateVerify() {
  ...
  const checkCandidateStatus = async () => {
    ...
    try {
      ...
      if (!isRegistered) {
        setMessage('Your wallet is not registered as a candidate. Please register first.');
        setMessageType('danger');
        setCheckingStatus(false);
        return;
      }

      const info = await deployedContract.methods.getCandidateInfo(walletAddress).call();
      ...
      setMessage('Failed to check candidate status: ' + error.message);
      setMessageType('danger');
    }
    setCheckingStatus(false);
  };
  ...
  if (!walletAddress || !candidateInfo || candidateInfo.status === 'VERIFIED') {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
        <Navbar title="VoteChain - Candidate Verification" />
        ...
        <div style={{ maxWidth: '600px', margin: '3rem auto', padding: '0 1rem' }}>
          <div style={{
            ...
            {candidateInfo?.status === 'VERIFIED' ? (
              <>
                <div style={{ fontSize: '3rem', marginBottom: '1rem' }}>✓</div>
                <h2>Already Verified</h2>
                <p style={{ color: '#6c757d' }}>Your candidate account is already verified. You can now participate in elections.</p>
              </>
            ) : (
              <>
                <h2>Access Denied</h2>
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f8f9fa' }}>
      <Navbar 
        title="BlockVote - Candidate Verification" 
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '350px 1fr', gap: '2rem' }}>
          ...
          <div>
            <div style={{
              ...
              <h3 style={{ marginBottom: '1.5rem', color: '#1e3a5f' }}>Candidate Information</h3>
              <div style={{ display: 'grid', gap: '1rem' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', color: '#6c757d', marginBottom: '0.25rem' }}>Name</div>
                  <div style={{ fontWeight: '500' }}>{candidateInfo.name}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: '#6c757d', marginBottom: '0.25rem' }}>Party</div>
                  <div style={{ fontWeight: '500' }}>{candidateInfo.party}</div>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', color: '#6c757d', marginBottom: '0.25rem' }}>Email</div>
                  <div style={{ fontWeight: '500' }}>{candidateInfo.email}</div>
                </div>
                <div>
                  ...
                  <span style={{
                    ...
                    borderRadius: '0.25rem',
                    backgroundColor: candidateInfo.status === 'VERIFIED' ? '#d1e7dd' : '#fff3cd',
                    color: candidateInfo.status === 'VERIFIED' ? '#0f5132' : '#664d03',
                    fontSize: '0.875rem',
                    fontWeight: '500'
                  }}>
                ...
                <div>
                  ...
                  <div style={{ fontWeight: '500' }}>{candidateInfo.registeredAt.toLocaleDateString()}</div>
                </div>
              </div>
